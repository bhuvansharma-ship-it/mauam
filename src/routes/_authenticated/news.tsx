  ...
  head: () => ({
    meta: [
      { title: "Latest News & Emergency Updates — Mausam" },
      {
        name: "description",
        content:
          "Verified real-time news on weather, disasters, public safety, and government advisories for your location.",
      },
      { property: "og:title", content: "Latest News & Emergency Updates — Mausam" },
      {
        property: "og:description",
        content:
          "Verified real-time news on weather, disasters, public safety, and government advisories.",
      },
    ],
  }),
  component: NewsPage,
...
function NewsPage() {
  ...
  const allQuery = useQuery(newsQueryOptions({ location: active, category: "All", q: "" }));
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <label htmlFor="news-severity" className="sr-only">
            Filter by severity
          </label>
          <select
            id="news-severity"
          ...
          >
            ...
            {(["breaking", "critical", "advisory", "official", "info"] as NewsSeverity[]).map(
              (s) => (
          ...
          {query.isLoading ? (
              ...
              {filtered.length > pageSize && (
                <div className="flex justify-center">
                  <button
                    ...
                    className="rounded-full border border-glass-border/60 bg-glass px-5 py-2.5 text-sm font-medium hover:bg-accent/20"
                  >
                    Load more stories
                  </button>
                </div>
            ...
            <div className="rounded-3xl border border-dashed border-glass-border/60 p-12 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Try clearing your search, picking a different category, or switching location.
              </p>
              <button
                onClick={() => {
                  setQueryInput("");
                  navigate({ search: { category: "All", q: "", severity: "" } });
                }}
