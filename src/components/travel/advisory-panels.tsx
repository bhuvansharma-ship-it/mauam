    ...
    <GlassCard className={cn("border p-5 sm:p-6", style.ring)}>
      <div className="flex flex-wrap items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-2 font-display text-xl font-semibold sm:text-2xl">{advisory.headline}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {advisory.from.name} <ArrowRight className="inline h-3.5 w-3.5" aria-label="to" /> {advisory.to.name} · ~{advisory.distanceKm} km
          </p>
        </div>
      </div>
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm">
              {advisory.reasons.map((r, i) => (
                <li key={i} className="flex gap-2"><span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-current opacity-60" aria-hidden="true" />{r}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-1 text-sm">
              {advisory.tips.map((t, i) => (
                <li key={i} className="flex gap-2"><span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-current opacity-60" aria-hidden="true" />{t}</li>
              ))}
            </ul>
          </div>
        </div>
  ...
  return (
    <GlassCard className="p-5 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat icon={<Thermometer className="h-4 w-4" aria-hidden="true" />} label="High / Low" value={`${advisory.destination.tempMaxC}° / ${advisory.destination.tempMinC}°`} />
        <Stat icon={<CloudRain className="h-4 w-4" aria-hidden="true" />} label="Rain (24h)" value={`${advisory.destination.precipMm.toFixed(1)} mm`} />
        <Stat icon={<Wind className="h-4 w-4" aria-hidden="true" />} label="Wind / Gust" value={`${advisory.destination.windKph} / ${advisory.destination.windGustKph} km/h`} />
        <Stat icon={<Sunrise className="h-4 w-4" aria-hidden="true" />} label="Sunrise" value={fmtTime(advisory.destination.sunrise)} />
      </div>
    </GlassCard>
  );
}
        ...
        {advisory.daily.slice(0, 5).map((d) => (
          <div key={d.date} className="rounded-2xl border border-border/60 p-3">
            ...
            <div className="mt-1 flex items-center gap-3 text-[11px] text-muted-foreground">
              <span className="inline-flex items-center gap-1"><CloudRain className="h-3 w-3" aria-hidden="true" />{d.precipMm.toFixed(0)}mm</span>
              <span className="inline-flex items-center gap-1"><Wind className="h-3 w-3" aria-hidden="true" />{d.windKph}</span>
            </div>
          </div>
        ))}
      </div>
