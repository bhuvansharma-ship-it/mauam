...
export function NewsImage({ article, className = "" }: { article: Article; className?: string }) {
  ...
  return (
    ...
    >
      {article.image.icon === "storm" && (
        <Mountain className="absolute bottom-0 left-0 h-full w-full text-white/10" />
      )}
      <div
        className="absolute inset-0"
        style={{
          background: `radial-gradient(circle at 30% 40%, oklch(1 0 0 / 0.15), transparent 60%)`,
        }}
      />
