    ...
    >
      <div className="flex flex-col gap-3 rounded-[calc(1.25rem-4px)] bg-gradient-to-r from-news-breaking/20 via-transparent to-news-breaking/10 p-4 sm:flex-row sm:items-center sm:gap-6 sm:p-5">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-news-breaking px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white animate-pulse-alert">
            Breaking
          </span>
        </div>
        <div className="flex-1 space-y-1">
          {items.map((b) => (
            <a
              ...
              className="block font-display text-base font-semibold hover:underline sm:text-lg"
            >
              {b.headline}
              <span className="ml-2 text-xs font-normal text-muted-foreground">
                · {b.source.name} · {timeAgo(b.publishedAt)}
              </span>
            </a>
          ))}
        </div>
