...
export function NewsCard({
  ...
  return (
    ...
    >
      ...
      <NewsImage
        article={article}
        className={size === "lg" ? "h-56 sm:h-72" : size === "sm" ? "h-32" : "h-44"}
      />
...
export function NewsCardCompact({ article }: { article: Article }) {
  ...
  const cls =
    "group flex gap-3 rounded-2xl border border-glass-border/50 bg-glass p-2.5 transition hover:border-primary/40";
  return article.url ? (
    <a href={article.url} target="_blank" rel="noopener noreferrer" className={cls}>
      {inner}
    </a>
  ) : (
    <Link to="/news/$slug" params={{ slug: article.slug }} className={cls}>
      {inner}
    </Link>
  );
}
