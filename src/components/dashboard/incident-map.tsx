...
const colorFor = {
  flood: "text-weather-flood bg-weather-flood/15",
  fire: "text-weather-critical bg-weather-critical/15",
  accident: "text-weather-warning bg-weather-warning/15",
  outage: "text-weather-storm bg-weather-storm/15",
};
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-glass-border/60">
          ...
          >
            ...
            {/* fake water */}
            <path
              d="M0 40 Q30 30 60 42 T100 38 L100 56 L0 56 Z"
              fill="oklch(0.3 0.08 240)"
              opacity="0.8"
            />
            {/* fake roads */}
          ...
          {incidents.map((inc) => {
            ...
            return (
              ...
              >
                <div
                  className={`grid h-9 w-9 place-items-center rounded-full border border-white/20 shadow-lg animate-pulse-alert ${colorFor[inc.kind]}`}
                >
          ...
          <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5 rounded-xl bg-background/70 p-2 text-[10px] backdrop-blur">
            {(["flood", "fire", "accident", "outage"] as const).map((k) => {
              ...
              return (
                <span
                  key={k}
                  className={`flex items-center gap-1 rounded-full px-1.5 py-0.5 ${colorFor[k]}`}
                >
