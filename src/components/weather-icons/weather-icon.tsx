...
export function WeatherIcon({ condition, className, animated = true }: { condition: Condition; className?: string; animated?: boolean }) {
        ...
        <svg viewBox="0 0 120 120" className={c}>
          ...
          <g className={animated ? "animate-float" : ""}>
            {Array.from({ length: 12 }).map((_, i) => {
              const a = (i * Math.PI) / 6;
              return <line key={i} x1={60 + Math.cos(a) * 34} y1={60 + Math.sin(a) * 34} x2={60 + Math.cos(a) * 48} y2={60 + Math.sin(a) * 48} stroke="var(--color-weather-sunny)" strokeWidth="3.5" strokeLinecap="round" opacity="0.85" />;
            })}
    ...
    case "partly-cloudy":
      return (
        <svg viewBox="0 0 120 120" className={c}>
          ...
          <circle cx="42" cy="42" r="20" fill="url(#pc-sun)" className={animated ? "animate-float" : ""} />
    ...
    case "rain":
      return (
        <svg viewBox="0 0 120 120" className={c}>
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            <line key={i} x1={38 + i * 12} y1={70} x2={34 + i * 12} y2={100} stroke="var(--color-weather-rain)" strokeWidth="3" strokeLinecap="round" style={animated ? { animation: `rain-fall 1.2s linear ${i * 0.15}s infinite` } : undefined} />
          ))}
        </svg>
      );
    case "storm":
      return (
        <svg viewBox="0 0 120 120" className={c}>
          ...
          <path d="M58 68 L48 90 L60 90 L52 108 L74 82 L62 82 L70 68 Z" fill="var(--color-weather-sunny)" className={animated ? "animate-pulse-alert" : ""} />
        </svg>
      );
    case "snow":
      return (
        <svg viewBox="0 0 120 120" className={c}>
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            <text key={i} x={30 + i * 15} y={90 + (i % 2) * 8} fill="var(--color-weather-snow)" fontSize="18">❄</text>
          ))}
        </svg>
      );
    case "fog":
      return (
        <svg viewBox="0 0 120 120" className={c}>
          {[0, 1, 2, 3].map((i) => (
            <rect key={i} x={20} y={40 + i * 14} width="80" height="6" rx="3" fill="oklch(0.85 0.01 250)" opacity={0.85 - i * 0.15} />
          ))}
        </svg>
      );
  }
